...
llms.get('/', (c) =>
  c.text(
    [
      ...
      '',
      `<script defer src="${SITE_URL}/script.js"></script>`,
      '',
      'That is the whole setup. The script is a few hundred bytes, sets no cookies, reads no storage and needs no config. It sends one beacon per pageview and follows client-side navigation on single-page apps on its own.',
      '',
      `The script posts events to ${API_URL}/api/event on the same origin it was loaded from, so the self-hosted build works with no changes. Events from localhost, 127.0.0.1 and file:// pages are ignored, so local development does not pollute your stats. Add a new website in the dashboard first, then drop the snippet in and stats appear within seconds of the first real visit.`,
      '',
      '## REST API',
      '',
