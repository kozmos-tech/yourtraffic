...
import { clientIp, country, newApiKey, newId, normalizeDomain, parseUA, referrerHost, visitorHash, } from '../lib/track.js';
// Public REST API: pageview ingest, stats reads and project management.
export const api = new Hono();
api.get('/health', (c) => c.json({ status: 'ok', service: 'yourtraffic', version: '0.1.0' }));
/* ---------------------------------------------------------------- ingest --- */
...
/* ----------------------------------------------------------------- stats --- */
const PERIOD_DAYS = { '24h': 1, '7d': 7, '30d': 30, '12mo': 365 };
...
async function computeStats(proj, period, group, by) {
    const days = PERIOD_DAYS[period];
    if (!days)
        return { ok: false, status: 400, error: 'Unknown period.' };
    const grp = group ?? (days > 90 ? 'month' : 'day');
    if (!GROUPS.has(grp))
        return { ok: false, status: 400, error: 'Unknown group.' };
    ...
    if (by) {
        ...
        if (!col)
            return { ok: false, status: 400, error: 'Unknown by.' };
...
api.get('/v1/stats', async (c) => {
    ...
    const result = await computeStats(proj, c.req.query('period') ?? '7d', c.req.query('group'), c.req.query('by'));
    if (!result.ok)
...
api.post('/projects', async (c) => {
    ...
    if (existing)
        return c.json({ error: 'That domain is already tracked.' }, 409);
    const row = {
        id: newId(),
        userId: u.id,
...
async function ownedProject(headers, id) {
    const u = await currentUser(headers);
    if (!u)
        return { user: null, proj: null };
    const proj = await db.query.project.findFirst({
