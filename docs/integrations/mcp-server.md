...
- `get_stats`. Returns stats for one website. Takes `site` (the domain), an optional `period` (`24h`, `7d`, `30d`, `12mo`) and an optional `by` breakdown (`page`, `referrer`, `country`, `browser`, `os`, `device`, `event`).
